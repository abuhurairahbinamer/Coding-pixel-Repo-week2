...
if(isEditing){
 ...
 const show=isEditing?<div><input className="edit-input" autoFocus value={editText} onClick={(e) => e.stopPropagation()} onBlur={handleEditSave} onKeyDown={allowKeyDown} onChange={updateValue}></input></div>:<span className="task-title"  onClick={(e) => e.stopPropagation()}  onDoubleClick={allowInlineEditing}>{task.title}</span>;
